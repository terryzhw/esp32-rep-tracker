...
with open("main/model.h", "w") as f:
    f.write("#ifndef MODEL_H\n#define MODEL_H\n\n")

    f.write(f"// Normalization params (min/max per feature: roll, pitch)\n")
    f.write(f"static const float NORM_MIN[] = {{{feat_min[0]:.4f}f, {feat_min[1]:.4f}f}};\n")
    f.write(f"static const float NORM_MAX[] = {{{feat_max[0]:.4f}f, {feat_max[1]:.4f}f}};\n")
